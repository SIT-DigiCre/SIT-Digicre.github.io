...
6. `metadata` を設定し、`noindex` は削除する。
7. ページ遷移後にスクロールするアンカーリンクを `id` 付与で調整。`Scroll` ボタン(リンク)は hero 内に配置。
8. `temp` ディレクトリは最終的に参照しないが、削除はしない。
9. ESLint/TSC チェック、`pnpm lint` or `pnpm run lint` を実施。
